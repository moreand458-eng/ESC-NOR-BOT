...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  if (!text) {
    let contoh = `${usedPrefix + command} سورة الكهف بصوت ماهر المعيقلي 360 mp4`
    return m.reply(
      `💀 *كيفية الاستخدام:*\n` +
  ...
  try {
    ...
    let quality = '360'  // الافتراضي
    let format = 'mp4'   // الافتراضي

    if (isQuality) {
      quality = lastArg.replace(/p$/, '')
      if (['mp4', 'mp3'].includes(secondLastArg)) {
    ...
    let query = args
      .filter(x => x !== lastArg && x !== secondLastArg) // إزالة الجودة والصيغة لو كانت موجودة
      .join(' ')
      .trim()

    let urlYT = ''

    // تحقق اذا query رابط يوتيوب
