...
let handler = async (m, { args, usedPrefix, command }) => {
	...
	} else throw `مثال: ${usedPrefix + command} id مرحبًا أنا روبوت`;
	try {
		const prompt = text.trim();
		let res = await translate(prompt, lang);
		let lister = Object.keys(await langList());
		let supp = `خطأ: اللغة "${lang}" غير مدعومة`;
...
handler.help = ["ترجم", "translate"].map((v) => v + " *النص*");
handler.tags = ['tools'];
handler.command = /^(tran(slate)|tr?|ترجم)$/i
