let handler = async (m, { conn, usedPrefix, command }) => {
			...
			await conn.sendMessage(m.chat, { video: { url: dir[Math.floor(Math.random() * dir.length)] }, caption: `*تفصل خطبة و اذكر الله وصلي على سيدنا محمد صلى الله عليه وسلم 💛*` }, { quoted: m })
	}

handler.help = ['storyanime']
handler.tags = ['anime']
handler.command = /^(خطبة)$/i
handler.limit = false
