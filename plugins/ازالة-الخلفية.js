...
let handler = async (m, {
    ...
    if (!mime) throw '*اعمل رد على صوره التي تريد ازالة خلفيتها 😉*'
    let img = await q.download()

    let cap = `*تم ازالة خلفية الصورة بواسطة إسِــــکْأّنِـوٌر بوت* 😘😗`
    try {
        await m.reply(wait)
...
handler.help = ['اوبيتو']
handler.tags = ['اوبيتو']
handler.command = /^(ازالة-الخلفية|removeb(ack)?g(round)?2)$/i
handler.limit = true
